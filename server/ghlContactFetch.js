...
import { getGhlCachedStale, setGhlCached, withGhlCache } from "./ghlResponseCache.js";
import { fetchGhlOpportunity, findGhlOpportunityByContactAndMls, opportunityAgentType, opportunityMatchesMls, opportunityMls, readOpportunityField, searchGhlOpportunitiesByMls, sortOpportunitiesNewestFirst, } from "./ghlOpportunityFetch.js";
...
function stripFieldKeyPrefix(raw) {
    let s = raw.trim();
    if (!s)
        return s;
    if (s.startsWith("contact."))
        s = s.slice("contact.".length);
    return s;
}
...
export async function getGhlCustomFieldDefinitions(force = false) {
 ...
 */
export async function updateGhlContactFields(contactId, fields) {
    const token = process.env.GHL_BEARER_TOKEN?.trim();
    if (!token)
    ...
    const r = await axios.put(`${ghlApiBase()}/contacts/${encodeURIComponent(contactId)}`, { customFields }, {
        headers: {
            Authorization: `Bearer ${token}`,
            Version: ghlApiVersion(),
...
function mapSearchHit(data, defs) {
    const customArr = Array.isArray(data.customFields)
        ? data.customFields
        : [];
    const raw = flattenCustomFields(customArr, defs);
    const pick = (key) => raw[key] ?? raw[key.toLowerCase()] ?? null;
    return {
...
async function searchGhlContactsUncached(query, limit) {
    ...
    for (let attempt = 0; attempt < retryDelaysMs.length; attempt++) {
        ...
        if (delay > 0)
            await ghlSleep(delay);
        const searchRes = await axios.post(`${ghlApiBase()}/contacts/search`, {
            locationId,
            page: 1,
 ...
 */
export async function searchGhlContacts(query, limit = 12) {
    const q = query.trim();
    if (q.length < 2)
        return [];
    const cacheKey = `contact-search:v1:${q.toLowerCase()}:${Math.min(Math.max(limit, 1), 25)}`;
    const { fresh, stale } = getGhlCachedStale(cacheKey, GHL_CONTACT_SEARCH_STALE_MS);
    if (fresh)
        return fresh;
    try {
...
async function ghlSearchWithBody(body, defs, limit) {
    const token = process.env.GHL_BEARER_TOKEN?.trim();
    if (!token)
        throw new Error("ghl_not_configured");
    ...
    if (searchRes.status < 200 || searchRes.status >= 300) {
        const bodyText = typeof searchRes.data === "string"
            ? searchRes.data.slice(0, 200)
            : JSON.stringify(searchRes.data).slice(0, 200);
...
async function scanGhlContactsForMls(locationId, mlsNorm, defs, limit) {
    const token = process.env.GHL_BEARER_TOKEN?.trim();
    if (!token)
        throw new Error("ghl_not_configured");
    const matched = [];
...
async function contactHitsFromOpportunitiesForMls(mls, limit) {
    ...
    for (const opp of opps) {
        ...
        if (fromOpps.length >= limit)
            break;
    }
    return fromOpps;
}
async function overlayFromGlobalMlsOpportunity(hit, mls) {
    const mlsNorm = normalizeMlsValue(mls);
    try {
        const global = await searchGhlOpportunitiesByMls(mls, 8);
        const match = sortOpportunitiesNewestFirst(global.filter((o) => opportunityMatchesMls(o, mlsNorm)))[0];
        if (!match?.id)
            return null;
...
async function searchGhlContactsByMlsUncached(mls, limit = 12) {
    const token = process.env.GHL_BEARER_TOKEN?.trim();
    const locationId = ghlLocationId();
    if (!token || !locationId)
        throw new Error("ghl_not_configured");
    ...
    for (const body of filterBodies) {
        try {
            ...
            if (matched.length > 0)
                return enrichContactHitsFromOpportunity(matched, mls);
        }
        catch {
    ...
    if (scanned.length > 0)
        return enrichContactHitsFromOpportunity(scanned, mls);
    try {
...
export async function searchGhlContactsByMls(mls, limit = 12) {
    const mlsNorm = normalizeMlsValue(mls);
    if (mlsNorm.length < 3)
        return [];
...
function parseListingAddressTail(line) {
    const parts = (line || "")
        .split(",")
        .map((p) => p.trim())
        .filter(Boolean);
...
function overlayOpportunityOnPrefill(hit, opp, requestedMls) {
    const pick = (key) => readOpportunityField(opp, key);
    const listingType = pick("listing_type") ?? pick("listing type") ?? pick("Listing Type") ?? hit.listingType;
    ...
    return {
        ...
        listingAddress,
        listingPhotoUrl: pick("listing_photo_url") ?? pick("listing_image_url") ?? hit.listingPhotoUrl,
        ...
        subdivisionHomeOwners: pick("subdivision_home_owners"),
        oneFourthMileHomeOwners: pick("one_fourth_mile_home_owners") ?? pick("14_mile_home_owners"),
...
async function enrichContactHitFromOpportunity(hit, mls) {
    const mlsNorm = normalizeMlsValue(mls);
    ...
    try {
        ...
        for (const row of global) {
            if ((row.contactId || "").trim() !== hit.id.trim())
                continue;
            try {
...
async function enrichContactHitsFromOpportunity(hits, mls) {
    const out = [];
...
async function prefillFromContactId(contactId) {
    ...
    const listingType = c.raw.listing_type ??
        c.raw["listing type"] ??
        c.raw.listingtype ??
        null;
    const agentType = c.raw.agent_type ??
        c.raw.agent_role ??
        c.raw["agent type"] ??
        c.raw.bor_s ??
        null;
    const photo = c.fields.listing_photo_url ??
 ...
 */
async function fetchGhlContactPrefillUncached(contactId, requestedMls) {
    ...
    try {
        ...
        const match = sortOpportunitiesNewestFirst(global.filter((o) => (o.contactId || "").trim() === contactId.trim() &&
            opportunityMatchesMls(o, normalizeMlsValue(want))))[0];
        if (match?.id) {
        ...
        for (const hit of global) {
            ...
            try {
                ...
                if ((full.contactId || "").trim() === contactId.trim() &&
...
export async function fetchGhlContactPrefill(contactId, requestedMls) {
