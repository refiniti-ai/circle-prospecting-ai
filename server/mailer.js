...
export function getMailTransportInfo() {
    ...
    return {
        configured: false,
        mode: "none",
        setupHint: "On the API host (e.g. Cloud Run), set GHL_MAIL_WEBHOOK_URL, or RESEND_API_KEY + RESEND_FROM (verified domain), or SMTP_HOST + SMTP_USER + SMTP_PASS.",
    };
}
function firstEmailFromList(raw) {
    const part = raw
        .split(/[,;]/)
        .map((s) => s.trim())
        .find((s) => s.includes("@"));
 ...
 */
async function sendViaResend(apiKey, to, subject, text, html) {
    const from = process.env.RESEND_FROM?.trim() || "Circle Prospecting AI <onboarding@resend.dev>";
    const recipients = to
        .split(/[,;]/)
        .map((s) => s.trim())
        .filter((s) => s.includes("@"));
    if (recipients.length === 0)
        throw new Error("Resend: no valid recipient");
...
async function sendViaGhlMailWebhook(to, subject, text, html, ghlExtras) {
...
export async function sendMarketingEmail(to, subject, text) {
    return sendTextEmail(to, subject, text);
}
/** @param html Optional HTML body; plain-text clients and spam filters still get `text`. */
export async function sendTextEmail(to, subject, text, html, options) {
    ...
    if (!host || !user || !pass) {
        console.warn("[MAILER:SKIPPED] No GHL_MAIL_WEBHOOK_URL, RESEND_API_KEY, or complete SMTP_* — transactional mail will not send. See GET /api/health mailSetupHint.");
...
function ghlWebhookFieldAliases(fields) {
    const out = { ...fields };
    const pairs = [
...
export function purchaseEmailLogoUrl() {
...
function purchaseDetailRow(label, value) {
    return `<tr>
...
function purchaseDetailRowLink(label, url) {
...
export function formatPurchasePhone(raw) {
...
export function buildPasswordResetEmailContent(resetLink, kind) {
...
export function buildContactFormEmail(args) {
...
function purchaseTypeLabel(checkoutType) {
...
export function buildTeamOrderEmailCopy(args) {
    ...
    const html = customer.html.replace(/<p style="margin:0 0 16px;font-size:16px;line-height:1.55;color:#334155;">/, `${banner}<p style="margin:0 0 16px;font-size:16px;line-height:1.55;color:#334155;">`);
    return {
        subject,
        text,
        html,
