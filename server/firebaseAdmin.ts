...
function useApplicationDefaultCredentials(): boolean {
  if (process.env.FIREBASE_USE_APPLICATION_DEFAULT_CREDENTIALS?.trim() === "1") return true;
  if (process.env.FIREBASE_USE_ADC?.trim() === "1") return true;
  // Managed GCP runtimes (no service-account JSON env needed)
  if (process.env.K_SERVICE) return true; // Cloud Run / Cloud Functions (many)
  if (process.env.FUNCTION_TARGET) return true;
  if (process.env.GAE_SERVICE) return true;
  return false;
}
...
export function getFirestoreDb() {
  if (getApps().length === 0) {
    ...
    if (svc) {
      ...
      if (!projectId || !useApplicationDefaultCredentials()) return null;
