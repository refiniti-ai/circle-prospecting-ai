...
} from "../server/ghlOpportunityFetch.js";
...
function radiusRow(label: string, v: string | null | undefined) {
  return { ring: label, count: v?.trim() || "(empty)" };
}

function oppRadii(oppId: string, fields: Record<string, string | null>) {
  return {
    opportunityId: oppId,
...
async function readOppRadii(oppId: string) {
