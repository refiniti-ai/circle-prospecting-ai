...
import { resolveIntroListingByMls } from "../../lib/introMlsListing";
...
type Props = {
  ...
  /** When set (e.g. from `/99promo/search/agent/:phone`), auto-search and show listing cards. */
  initialAgentPhone?: string;
  /** Intro funnel only — return true to navigate to `/99promo/search/agent/:phone`. */
  onAgentPicked?: (phone: string) => boolean;
  className?: string;
};
...
export function IntroCampaignSearch({
  ...
  const onPickAgent = useCallback(
    async (hit: GhlContactSearchHit) => {
      const phone =
        normalizeAgentPhoneParam(hit.phone ?? "") || normalizeAgentPhoneParam(agentQuery);
      if (phone.length >= 10 && onAgentPicked?.(phone)) {
        return;
      }
    ...
    [agentQuery, onAgentPicked, reportError, setBusyState]
  );
  ...
  const onFindListing = useCallback(async () => {
    ...
    try {
      if (mlsQ) {
        ...
        const listing = await resolveIntroListingByMls(mlsQ);
        onResult({ kind: "listing", listing });
        return;
      }
